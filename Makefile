...
GOLDFLAGS?=-extldflags=-zrelro -extldflags=-znow -s -w -X github.com/kubermatic-labs/kubeone-remote/pkg/cmd.version=$(GITTAG) -X github.com/kubermatic-labs/kubeone-remote/pkg/cmd.commit=$(GITCOMMIT) -X github.com/kubermatic-labs/kubeone-remote/pkg/cmd.date=$(BUILD_DATE)
...
.PHONY: build
build: dist/kubeone-remote

.PHONY: vendor
vendor: buildenv
	go mod vendor

dist/kubeone-remote: buildenv
	go build -ldflags='$(GOLDFLAGS)' -v -o $@ .

dist/kubeone-remote-debug: buildenv
